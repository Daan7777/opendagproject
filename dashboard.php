...
if(isset($_GET['sector']) && !empty($_GET['sector'])){
    ...
    $sector = $_GET['sector'];
    $sql = "SELECT DISTINCT JobOrganisation, OrganisationURL, JobSector  FROM jobs where JobSector = '$sector' order by JobOrganisation";
    $sql2 = "select count(DISTINCT JobOrganisation) as aantal from jobs where JobSector = '$sector'";     
}
else
{
    $sql = "SELECT DISTINCT JobOrganisation, OrganisationURL, JobSector  FROM jobs order by JobOrganisation";
